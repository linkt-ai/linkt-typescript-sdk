...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['LINKT_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Linkt) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['LINKT_LOG'] = 'debug';
      const client = new Linkt({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['LINKT_LOG'] = 'not a log level';
      const client = new Linkt({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'LINKT_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Linkt({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['LINKT_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Linkt({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Linkt({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['LINKT_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Linkt({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Linkt({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Linkt({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['LINKT_API_KEY'] = 'My API Key';
    const client = new Linkt();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['LINKT_API_KEY'] = 'another My API Key';
    const client = new Linkt({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Linkt({ apiKey: 'My API Key', fetch: testFetch });
